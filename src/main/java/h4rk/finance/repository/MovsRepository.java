...
public class MovsRepository {
	...
	public int deleteMovs(long id) {
		return jdbcTemplate.update("DELETE FROM mov WHERE id = ?", id);
	}
	...
	public List<Mov> getMovs() {
		return jdbcTemplate.query("SELECT * FROM mov", (rs, rowNum) -> new Mov(rs.getLong("id"), rs.getString("description"), rs.getDouble("amount"), rs.getDate("date"), rs.getBoolean("isIncome")));
	}

	public Mov getMovById(long id) {
		return jdbcTemplate.queryForObject("SELECT * FROM mov WHERE id = ?", (rs, rowNum) -> new Mov(rs.getLong("id"), rs.getString("description"), rs.getDouble("amount"), rs.getDate("date"), rs.getBoolean("isIncome")), id);
	}
}
